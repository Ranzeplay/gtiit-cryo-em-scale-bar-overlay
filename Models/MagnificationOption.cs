namespace ScaleBarOverlay.Models;

public class MagnificationOption(int ratio, float pixels, int scaleBarNanometers)
{
    static MagnificationOption()
    {
        Services.AppLogger.Info(nameof(MagnificationOption), "MagnificationOption type initialized.");
    }

    public int Ratio { get; } = ratio;

    public float Pixels { get; } = pixels;

    public float PixelLength => 100f / (0.1f * Pixels);
    
    public int ScaleBarNanometers { get; } = scaleBarNanometers;

    public virtual string DisplayText => $"{Ratio}K";
    
    public class AutoMagnificationOption() : MagnificationOption(0, 0, 0)
    {
        public override string DisplayText => "Auto (fallback to 11K)";
    }
    
    public static MagnificationOption[] TemplateOptions
    {
        get
        {
            Services.AppLogger.Info(nameof(MagnificationOption), "Generating magnification template options.");
            return
            [
                new AutoMagnificationOption(),
                new(11, 13.3f, 500),
                new(36, 3.96f, 100),
                new(45, 3.17f, 100),
                new(57, 2.5f, 100),
                new(73, 1.9f, 100),
                new(92, 1.5f, 50),
                new(120, 1.2f, 50),
                new(150, 0.95f, 50),
                new(190, 0.74f, 50),
                new(240, 0.58f, 50),
            ];
        }
    }
}

